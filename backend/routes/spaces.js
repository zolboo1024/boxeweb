...
router.route('/').get((req, res) => { //auth is the authorization middleware that we
  ...
  //classes can be actually used as a constructor and as an object at the same time.
    .then(spaces => res.json(spaces)).catch(err => res.status(400).json('Error: ' + err));
});
...
router.route('/:id').delete(auth, (req, res) => {
  Space.findByIdAndDelete(req.params.id).then(() => res.json("Space deleted!")) //res is what is returned. If it returns a message, we
  //don't really care
    .catch(err => res.status(400).json('Error: ' + err));
});
...
router.route('/update/:id').post(auth, (req, res) => {
  Space.findById(req.params.id).then(spaceToUpdate => {
    ...
    spaceToUpdate.save().then(() => res.json("Space saved!")).catch(err => res.status(400).json('Error: ' + err));
