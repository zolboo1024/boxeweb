...
class SpacesList extends Component {
  ...
  componentDidMount() {
    ...
    axios.get('/spaces/', tokenConfigJS(this.props.token)).then(response => {
  ...
  deleteSpace(id) {
    axios.delete('/spaces/' + id).then(response => {
  ...
  spaceList() {
    return this.state.spaces.map(currentspace => {
      return <CollectionItem className="avatar" onMouseEnter={() => {
        ...
        }} onMouseLeave={this.onMouseExit} key={currentspace._id} href={'/spaces/'+currentspace._id}>
        <img alt="" className="circle" 
        src={"/spaces/images/" + currentspace.imagename}/>
