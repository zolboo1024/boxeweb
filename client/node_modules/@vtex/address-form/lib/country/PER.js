...
var _postalCodes = require('../transforms/postalCodes');
...
exports.default = {
  country: 'PER',
  abbr: 'PE',
  postalCodeFrom: _constants.THREE_LEVELS,
  postalCodeLevels: ['state', 'city', 'neighborhood'],
  thirdLevelPostalCodes: (0, _postalCodes.thirdLevelPostalCodes)(countryData),
  fields: [{
    hidden: true,
    name: 'country',
    maxLength: 100,
    ...
    level: 3,
    basedOn: 'city',
    optionsMap: (0, _addressFieldsOptions.getThreeLevels)(countryData)
  }, {
    name: 'receiverName',
  ...
  geolocation: {
    postalCode: {
      ...
      handler: function handler(address) {
        ...
        if (countryData[address.state.value] && countryData[address.state.value][address.city.value] && countryData[address.state.value][address.city.value][address.neighborhood.value]) {
          address.postalCode = {
    ...
    neighborhood: {
      ...
      handler: function handler(address) {
        if (address.neighborhood && (address.neighborhood.value === 'Distrito de Lima' || address.neighborhood.value === 'Lima')) {
    ...
    state: {
      ...
      handler: function handler(address) {
        ...
        var states = Object.keys(countryData);
        for (var i = 0; i < states.length; i++) {
          var state = states[i];
          var cities = Object.keys(countryData[state]);
          var hasCity = cities.indexOf(address.city.value) !== -1;
          if (hasCity) {
  ...
  summary: [[{ name: 'street' }, { delimiter: ' ', name: 'number' }, { delimiter: ' ', name: 'complement' }], [{ name: 'neighborhood' }], [{ name: 'city' }, { delimiter: ', ', name: 'state' }]]
};
